...
object FlagManager {
    private const val DATABASE_NAME = "ctf.db"
    private const val TABLE_NAME = "flag_parts"
...
//
//        val dbHelper = FileUtils(context, DATABASE_NAME, TABLE_NAME, COLUMN_NAME)
//        dbHelper.clearTable()
//
//        for (i in 0 until 4) {
...
//
//            dbHelper.insertFlagPart(encodedPart)
...
//    }
    ...
    fun retrieveFlag(context: Context): String {
        val dbHelper = FileUtils(context, DATABASE_NAME, TABLE_NAME, COLUMN_NAME)
        val parts = dbHelper.getFlagParts()
    ...
    private fun xorEncryptDecrypt(data: ByteArray, key: ByteArray): ByteArray {
        val repeatedKey = ByteArray(data.size) { index -> key[index % key.size] }
